 ...
 */
public class Shop implements Initializable, PathConstants {
    ...
    private static boolean itemBought(int a) throws IOException {
        ...
        return FileHandler.getFileContent().charAt(a - 1) == 'T';
    }

    private static int getSkinEquipped() {

        try {
            String[] splitted = Files.readString(FileHandler.filePath).split(" ");
    ...
    private void itemChecker(String a, int b) throws NumberFormatException, IOException {
        ...
        if (price.getText().equals(a)) {
            if (FileHandler.getFileContent().charAt(b) == 'T') {
                String[] full = Files.readString(FileHandler.filePath).split(" ");
                ...
                Files.writeString(FileHandler.filePath, writeString);

                notification.setText("Skin " + getSkinEquipped() + " and Map " + noMap() + " has been selected");
                owned.setText("Skin Equipped");

            } else if (FileHandler.getFileContent().charAt(b) == 'F' && FileHandler.canBuy(Integer.parseInt(a))) {
                StringBuilder sb = new StringBuilder(FileHandler.getFileContent());
                sb.setCharAt(b, 'T');
                ...
                Files.writeString(FileHandler.filePath, str);
                notification.setText("You have successfully purchased Skin " + (b + 1) + " :D");
                owned.setText("Equip");
            } else if (!FileHandler.canBuy(Integer.parseInt(a))) {
